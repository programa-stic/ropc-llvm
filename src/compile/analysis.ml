...
let instr_type = function
    ...
    (* Este es el tipo de instrucción más básco. Los tipos T1, T2 y T3, se
     * reescriben con estás funciones (mediante la función
     * *make_implement.implement*.)
     *)
    | Lbl(_)
    | Comment(_)
    | AdvanceStack(_)
...
let find_all_gmetas instr gms =
    ...
    let is_opstack g =
        match g with
        | Common.OpEsp(_,_,_) -> true
        | _ -> false
    in
    let get_stack_fix gm =
        let GMeta(_,_,_,sf) = gm
        in sf
    in
    let f_match_movreg r g gm =
        match g with
        | Common.LoadConst(gr,_) -> gr=r
        | _ -> false
    in
    let f_match_op_esp op r g gm =
        match g with
        | Common.OpEsp(gop, gr, _) -> (gop=op && gr=r)
        | _ -> false
    in
    let f_match =
        match instr with
        ...
        | _ ->
            fun g gm -> false (* assert false *) (* SACAR! *)
    in
    let matching_gms = find_gms f_match in
        if matching_gms = [] then
            raise Not_found
        else
            matching_gms
...
(*
            ...
            let tmp_mov  = MovRegReg(tmp_reg, r) in
            let wm       = WriteM(addr_reg, tmp_reg) in
            [ la ; tmp_mov ; wm ]
*)
    ...
    in
    let implement instr =
        (* type2idx : Retorna un índice a partir de un tipo de instrucción.
         * Para hacer usada con la lista de funciones de implementación.
         *)
        let type2idx typ =
            match typ with
            | T3 -> 2
            ...
            | T0 -> assert false
        in
        (* init : Inicializa las funciones de implementación. *)
        let init () =
            ...
            funs
        in
        (* funs : Funciones de implementación de instrucciones. *)
        let funs = init () in
...
(* Retona la posicion de un argumento, indexando desde cero, para las
...
let possible_regs_t0 gms instr =
    ...
    let make_sets groups =
        let f acc l =
            let set = RegSets.set_from_list l in
            set::acc
        in
...
let make_possible_regs_funs gadgets implement =
    ...
    let rec possible_regs_by_pos gadgets implement instr pos =
        ...
        if (cache_test instr pos) then
            ...
            let _ = Printf.printf "make_possible_regs_funs :: LENGTH: %d ; IDX: %d\n" (List.length reg_set_list) pos in
            let _ = Printf.printf "  instr: %s\n" (dump_instr instr) in
            let _ = (List.iter (fun s -> (dump_reg_set s)) reg_set_list) in
            let _ = Printf.printf "\n" in
    ...
    possible_regs_by_arg gadgets implement instr arg =
        ...
        (* let _ = Printf.printf "dumping instr: %s\n" (dump_instr instr) in (* DEBUG*) *)
...
(* Para una instrucción dada, retorna un par (mod_regs, read_regs) donde: *)
(* mod_regs: es una lista de los registros que se modifican *)
(* read_regs: es una lista de los registros que se leen *)
...
let analyse_reads_writes instrs =
    ...
    (* Actuliza las tablas de hash con reg->i, donde i indica el número de orden
    en que se escribe o se lee reg (el nro de orden es realitivo a la cantidad de
    instrucciones que se están analizando). *)
    let update_hashes i instr reads writes =
        (* wr: lista de registros que se escriben durante la ejecucion de la instr *)
        let wr = mod_vars instr in
        (* rd: lista de registros que se leen durante la ejecucion de la instr *)
...
(* Retorna tabla de hash de tipo [int, SRegSet] *)
let inverse_hash h =
...
(* Retorna un conjunto de registros que son leidos pero no escritos. *)
 ...
 * Vars are in conflict, when they can't share a register (they are both alive
 * at the same time).
 *)
let calc_conflicts pairs =
    (* Procesa todos los pares (sreg, alive set) y retorna un hash que tiene,
     * para cada sreg su alive set (un collect).
     *)
    let f h (_, alive) =
        let g acc sreg =
            (* hash_get : retorna h[sreg]. Si existe la clave *sreg* retorna
             * *SRegSet.empty*.
             *)
            let cur = hash_get h sreg SRegSet.empty in
            (* Acumula en new_set los sreg *alive* con los que ya tenía en *h*
             * para *sreg*.
             *)
...
(* Retorna: *)
(* función assign_regs 0 : instr -> top_preserved -> s_pairs *)
(* donde: *)
(* s_pairs: lista (instr, gmets)*)
...
let make_assign_regs gmetas stack_ptr frame_ptr =
    ...
    (* p_by_arg : possible_regs_by_arg; p_by_pos : possible_regs_by_pos *)
    ...
    let rec assign_regs depth instrs top_preserved =
        ...
        (* collect_all_vars : Retorna una lista de symbolic regs para
         * una lista de instrucciones dada.
         *)
        ...
        let satisfy perms pairs =
            ...
            let satisfy_t0 instr preserved =
                ...
                let possible_gmetas =
                    try
                        find_all_gmetas instr gmetas
                    with Not_found ->
                        printf "[+] possible_gmetas :: Not Found.\n";
                        []
                    in
            ...
            let rec aux perms =
                ...
                | f_assign::tl ->
                    begin
                    (*
                    let _ = Printf.printf "[+] Testing assignment : \n" in
                    let _ = List.iter (fun sr -> Printf.printf "%s\n" sr) f_assign in
                    let _ = Printf.printf "======================== \n" in
                    *)
                    (* Concretize all regs *)
                    ...
                    try
                        ...
                        f_assign, impl
                    with Not_found ->
                        dprintf depth (fun _ -> Printf.printf "[-] No assignment found.\n");
                        List.map print_pair pairs;
                        aux tl
                    end
                | [] ->
                    (* No assignment is satisfiable *)
                    dprintf depth (fun _ -> Printf.printf "[-] No assignment is satisfiable.\n");
                    raise Not_found
            in
        ...
        let dump vars possible conflicts perms top_preserved pairs =
            let print_pair (instr, alive) =
                let _ = dprintf depth (fun _ -> Printf.printf "[+] Procesando instrucción : '%s'\n" (dump_instr instr)) in
                let _ = dprintf depth (fun _ -> Printf.printf "%s alive: " (dump_instr instr)) in
                let _ = dump_sreg_set alive in
                let _ = Printf.printf "\n" in
                let _ = dprintf depth (fun _ -> Printf.printf "============================\n") in
                print_newline ()
            in
        ...
        let _ =
            dprintf depth (fun _ -> Printf.printf "[+] assign_regs :\n");
            List.iter (fun ins -> dprintf depth (fun _ -> Printf.printf "    %s\n" (dump_instr ins))) instrs
        in


        (* Analiza para cada instrucción, qué registros (simbólicos y concretor)
         * están vivos (alguna instrucción lo está usando).
         *)
        let pairs = analyse_liveness instrs in

        (* Calcula sregs en conflicto. *)
        (* Return a hash: sreg -> set of conflicting sregs. *)
        ...
        let possible =
        try
            possible_regs p_by_arg instrs
        with
            Failure explanation -> print_endline explanation;
            failwith "Error!!";
            Hashtbl.create 1
        in
        (* Retorna una lista de symbolic regs para una lista de instrucciones dada. *)
