...
(* Retorna una Ast.exp con la asignación de var al valor value. value lo
    transforma en Ast.Int. *)
...
(* Retorna una variable con valor value generada para el registro reg. *)
...
(* Genera una lista de longitud num_dwords de (offset, value) donde value
es un valor de 32bits aleatorio. *)
(* offset, value *)
...
(* Retorna una lista de (reg, rand_value) donde reg in regs. *)
(* Tiene como restricción que :
    * el valor asignado a eax sea mayor que los valores asignados a los otros
    registros.
    * que eax/r1 != eax/r2 para todo par (r1, r2) tal que r1 != r2
*)
...
(* Retorna un par (lista de (reg, rand_value), lista de Ast.Move(var(reg), rand_value) )*)
let gen_init_and_rv_pairs' f_cast regs =
    (* rv_pairs = lista de (reg, rand_value) *)
    let rv_pairs = gen_rv_pairs regs in
    (* Init es una lista de Ast.Move(var(reg), rand_value) *)
...
(* Genera una lista inicial de (regs, rand_value) para todos los registros
    excepto ESP. *)
...
let g_copy_reg () =
    ...
    (* Asignación inicial de valores random a los registros *)
    let f_init stmts =
        init
    in

    let f_finish ctx =
        (* Retorna una lista (reg, val) del contexto de la ejecución simbólica
        ctx. *)
        let rv_new = get_regs_no_esp ctx in
        (* dst_src = lista de (reg, hash[value] *)
        let dst_src = collect rv_hash rv_new in
        (* delete trivial copies *)
        (* remueve aquellos que (r1, r2) tales que r1==r2 *)
